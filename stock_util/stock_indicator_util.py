        ...
        if len(final_df) > 0:
            ...
            if cnt > 0:
                ...
                for code in stock_names:
                    gs.remove_redis_update_stock_code(update_col, code)
                update_record = pd.DataFrame(max_dates, columns=['stock_code', f'{update_col}'])
                my.batch_insert_or_update(engine, update_record, record_table, 'stock_code')
...
def calculate_stock_obv(frequency, batch_size=10, window=30):
    ...
    for i in range(0, len(stock_list), batch_size):
        ...
        final_df = df[['stock_code', 'stock_name', 'stock_date', 'trading_volume', 'obv', '30ma_obv']].replace(
            {np.nan: None})
        if len(final_df) > 0:
            cnt = my.batch_insert_or_update(engine, final_df, insert_table, 'stock_code', 'stock_date', batch_size=1500)
            if cnt > 0:
